...
namespace PR_Top_Service_MVC.Controllers
{
    public class ConfirmPostulationController : Controller
    {
        ...
        {
            IQueryable<Postulation> postulation =from Postulation in _context.Postulations.Include(x=>x.IdProfessionalNavigation.IdPersonNavigation)  /*.Where(x => x.Status == 1);*/
                                                   ...
                                                   where Postulation.Status==1 && Person.IdDepartment==UserConfig.DepPer
                                                   select Postulation;
